...
const log = debug('whatsit:request');
...
class Requestable {
   /**
    * Either a username and password or an oauth token for WhatsIt
    * @typedef {Object} Requestable.auth
    * @prop {string} [username] - the WhatsIt username
   ...
   constructor(auth, apiBase, AcceptHeader) {
      // this.__apiBase = apiBase || 'https://api.whatsit.net';
      this.__apiBase = apiBase || 'https://api.whatsit.net';
      this.__auth = {
         token: auth.token,
   ...
   __getRequestHeaders(raw, AcceptHeader) {
      let headers = {
         ...
         'Accept': 'application/vnd.whatsit.' + (AcceptHeader || this.__AcceptHeader),
      };
    ...
    * @param {Object} request - the raw {@linkcode https://whatsit.com/mzabriskie/axios#response-schema Response}
    */
   /**
    * Make a request.
   ...
   /**
    * Make a request and fetch all the available data. WhatsIt will paginate responses so for queries
